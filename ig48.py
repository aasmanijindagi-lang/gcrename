...
async def get_secret(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    def do_login():
        ...
        try:
            ...
            # Warm-up requests to obtain cookies and device id
            cl.http.get("https://www.instagram.com/")              # home
            cl.http.get("https://www.instagram.com/accounts/login/")  # login page
            cl.get_timeline_feed()                                 # api warm-up
            cl.pre_login_flow()                                    # prepare tokens

            # ✅ Perform login
            result = cl.login(ig_username, password)

            # Optional: verify we actually got JSON back
            if not result:
                raise ValueError("Empty login response (possibly HTML)")
            ...
            # Print the raw response text for debugging next time
            try:

...
def get_storage_state_from_instagrapi(settings: Dict):
    ...
    with sync_playwright() as p:
        ...
        if 'accounts/login' in page.url or 'challenge' in page.url:
            raise ValueValue("Session transfer failed, login required")
...
async def get_secret(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    def do_login():
        ...
        # --- Fix: warm up session so csrftoken cookie exists ---
        try:
            cl.private_request("accounts/login/", method="GET")
        except Exception:
            pass
        # --------------------------------------------------------
        ...
        try:
            ...
            return None, {'type': '2fa', 'settings': settings}

        except ChallengeRequired:
            cl.challenge_resolve(cl.last_json, choice="1")
            settings = cl.get_settings()
            return None, {'type': 'challenge', 'settings': settings}

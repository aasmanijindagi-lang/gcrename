...
def get_storage_state_from_instagrapi(settings: Dict):
    ...
    # Do NOT override instagrapi's UA here (see earlier advice). Playwright will use its own UA.
    ...
    def _inner(p):
        ...
        if cookies:
            try:
                ...
                # ignore cookie add failures, continue
                pass
    ...
    return run_with_sync_playwright(_inner)
...
def list_group_chats(storage_state, username, password, max_groups=10):
    # We will run the Playwright logic inside the helper-thread
    def _inner(p, storage_state_inner):
        browser = p.chromium.launch(headless=True)
        context = browser.new_context(
            storage_state=storage_state_inner if storage_state_inner else {},
            user_agent=USER_AGENT,
            viewport={'width': 1080, 'height': 2340},
            screen={'width': 1080, 'height': 2340},
            device_scale_factor=2.625,
        ...
        try:
            ...
            if 'accounts/login' in page.url:
                # need to login via instagrapi and get new storage_state
            ...
            groups = []
            # iterate chat items (use safe count())
            total = chat_items.count()
            for i in range(min(20, total)):
                ...
                try:
                    ...
                    if not members_section.is_visible():
                        page.go_back()
                        continue
                    members = page.locator("//div[contains(text(),'Members')] / following-sibling::div //span")
    ...
    # call inner via helper thread
    return run_with_sync_playwright(_inner, storage_state)
...
def run_rename_loop(task_dict, user_data, account, thread_url, names_list, task_id, stop_event, user_id, secret):
    ...
    def _inner(p, storage_state_inner):
        ...
        context = browser.new_context(storage_state=storage_state_inner, user_agent=USER_AGENT,
                                     viewport={'width': 1080, 'height': 2340},
                                     screen={'width': 1080, 'height': 2340},
                                     device_scale_factor=2.625,
                                     is_mobile=True,
        ...
        # ---- yahan se tumhara pura existing code (login required, details pane, rename loop) as-is rahega ----
        try:
        ...
        # baaki ka poora code jo tumne bheja hai, wahi paste karo yahan par
        # (login required block, details pane, popup watcher, db setup, rename loop, etc.)
    ...
    run_with_sync_playwright(_inner, storage_state)
...
async def get_secret(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    def do_login():
            ...
            session_file = f"{ig_username}_session.json"
            ...
            try:
                ...
                cl.login(ig_username, password)
                cl.dump_settings(session_file)
                print("[DEBUG] Login successful!")

                # Convert session to Playwright storage state
